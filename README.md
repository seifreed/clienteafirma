...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  - **JAdES T/LT/LTA:** JAdES-T ya puede obtener un token RFC 3161 desde `tsaURL` o serializar en `etsiU` un token aportado por el llamador mediante JWS JSON Serialization flattened, verificando que el imprint del token selle la firma JWS; el certificado firmante se valida como X.509 vigente y la cadena `x5c` solo admite certificados X.509. Quedan la política TSA CTT por defecto y la conexión con `afirma-trust-tsl` para LT/LTA.
  - **TSL/LOTL real:** `LotlLoader` ya descarga la LOTL por HTTPS (<https://ec.europa.eu/tools/lotl/eu-lotl.xml>), verifica XMLDSig con clave pública pinada suministrada por el integrador y persiste cache local verificada con fallback si falla la red o llega una descarga no válida; `TrustListService` verifica firma y vigencia de las identidades de servicio y del certificado consultado. Queda fijar el pin oficial del certificado de firma de la Comisión y la política de distribución/rotación CTT.
  - **OID4VP completo:** perfiles finales de interoperabilidad. DCQL nativo ya sustituye al legacy `presentation_definition_uri` cuando se declara y exige `credentials` con `id`/`format`; JAR by value ya genera Request Objects firmados con `iss` y vigencia corta; `client_id`/`response_uri`/`presentation_definition_uri` exigen HTTPS, `nonce`/`state` no pueden ir vacíos y `response_mode` queda limitado a `direct_post`/`direct_post.jwt`; JARM `direct_post.jwt` ya se solicita y valida, incluidos `iss`, `aud`, `state`, `exp` obligatorio, `nbf` si existe y `presentation_submission` JSON.
  - **SD-JWT VC:** `SdJwtVerifier` ya valida issuer JWT contra TSL, vigencia del certificado emisor, `exp` obligatorio y `nbf` si existe, disclosures como arrays JSON `[salt, claim_name, claim_value]` referenciados por `_sd`, firma del Holder, `typ`, `iat`, `exp` obligatorio, `nbf` si existe, `aud`, `nonce` y `sd_hash` del Key Binding JWT.
