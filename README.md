...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  - **JAdES T/LT/LTA:** JAdES-T ya puede obtener un token RFC 3161 desde `tsaURL` o serializar en `etsiU` un token aportado por el llamador mediante JWS JSON Serialization flattened; el certificado firmante se valida como X.509 vigente y la cadena `x5c` solo admite certificados X.509. Quedan la política TSA CTT por defecto y la conexión con `afirma-trust-tsl` para LT/LTA.
  ...
  - **OID4VP completo:** perfiles finales de interoperabilidad. DCQL nativo ya sustituye al legacy `presentation_definition_uri` cuando se declara y exige `credentials`; JAR by value ya genera Request Objects firmados con `iss` y vigencia corta; `client_id`/`nonce` no pueden ir vacíos, `response_uri` exige HTTPS y `response_mode` queda limitado a `direct_post`/`direct_post.jwt` también en `EudiwProtocolHandler`; JARM `direct_post.jwt` ya se solicita y valida, incluidos `iss`, `exp`/`nbf`.
  - **SD-JWT VC:** `SdJwtVerifier` ya valida issuer JWT contra TSL, vigencia del certificado emisor y claims `exp`/`nbf`, disclosures como arrays JSON referenciados por `_sd`, firma del Holder, `typ`, `iat`, `aud`, `nonce` y `sd_hash` del Key Binding JWT.
