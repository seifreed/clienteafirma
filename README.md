...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  ...
  - **TSL/LOTL real:** `LotlLoader` ya descarga la LOTL por HTTPS (<https://ec.europa.eu/tools/lotl/eu-lotl.xml>), verifica XMLDSig con clave pública pinada suministrada por el integrador y persiste cache local verificada con fallback si falla la red o llega una descarga no válida; `TrustListService` verifica firma y vigencia de las identidades de servicio. Queda fijar el pin oficial del certificado de firma de la Comisión y la política de distribución/rotación CTT.
