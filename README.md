...
| **M4**  | eIDAS&nbsp;2 / EUDI Wallet — JAdES, TSL/LOTL, OID4VP, SD-JWT | 🟡 Esqueleto fase 2 — `afirma-crypto-jades` (B-B compact + JWS JSON Serialization; JAdES-T JSON con token RFC 3161 aportado por el llamador), `afirma-trust-tsl` (parser ETSI TS 119 612 + verificador XMLDSig + loader LOTL HTTPS + cache 24h + `TrustListService`), `afirma-eudiw-bridge` (OID4VP `AuthorizationRequest` con DCQL/JAR/JARM, SD-JWT VC parser/verifier, cliente HTTP), `EudiwProtocolHandler` cableado en `ProtocolInvocationLauncher` con deep-link wallet configurable. TSA real para JAdES-T, JAdES LT/LTA, pin oficial Comisión para LOTL, conformance EU Reference Wallet y coordinación móvil pendientes (TODO M4.x). |
...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  - **JAdES T/LT/LTA:** JAdES-T ya puede serializar en `etsiU` un token RFC 3161 aportado por el llamador mediante JWS JSON Serialization flattened. Quedan la obtención contra TSA real y la conexión con `afirma-trust-tsl` para LT/LTA. Bloqueante: política sobre TSA (decisión CTT).
  ...
  - **Mobile:** `EudiwProtocolHandler` ya permite `walletUri` para deep-link móvil configurable. Quedan contratos cerrados con `afirma-android` y `afirma-ios` (repos separados en CTT).
