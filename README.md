...
| **M4**  | eIDAS&nbsp;2 / EUDI Wallet — JAdES, TSL/LOTL, OID4VP, SD-JWT | 🟡 Esqueleto fase 2 — `afirma-crypto-jades` (B-B compact JWS), `afirma-trust-tsl` (parser ETSI TS 119 612 + verificador XMLDSig + cache 24h + `TrustListService`), `afirma-eudiw-bridge` (OID4VP `AuthorizationRequest` con DCQL/JAR/JARM, SD-JWT VC parser/verifier, cliente HTTP), `EudiwProtocolHandler` cableado en `ProtocolInvocationLauncher`. Niveles JAdES T/LT/LTA, integración con LOTL real, conformance EU Reference Wallet y coordinación móvil pendientes (TODO M4.x). |
