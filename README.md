...
| **M4**  | eIDAS&nbsp;2 / EUDI Wallet — JAdES, TSL/LOTL, OID4VP, SD-JWT | 🟡 Esqueleto fase 2 — `afirma-crypto-jades` (B-B compact JWS), `afirma-trust-tsl` (parser ETSI TS 119 612 + verificador XMLDSig + cache 24h + `TrustListService`), `afirma-eudiw-bridge` (OID4VP `AuthorizationRequest` con DCQL/JAR, SD-JWT VC parser/verifier, cliente HTTP), `EudiwProtocolHandler` cableado en `ProtocolInvocationLauncher`. Niveles JAdES T/LT/LTA, integración con LOTL real, JARM, conformance EU Reference Wallet y coordinación móvil pendientes (TODO M4.x). |
...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  ...
  - **OID4VP completo:** soporte JARM (RFC 9207) y perfiles finales de interoperabilidad. DCQL nativo ya sustituye al legacy `presentation_definition_uri` cuando se declara; JAR by value ya genera Request Objects firmados.
