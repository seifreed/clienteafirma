...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  ...
  - **SD-JWT VC:** `SdJwtVerifier` ya valida issuer JWT contra TSL, vigencia del certificado emisor y claims `exp`/`nbf`, disclosures como arrays JSON referenciados por `_sd`, firma del Holder, `typ`, `iat`, `exp`/`nbf`, `aud`, `nonce` y `sd_hash` del Key Binding JWT.
