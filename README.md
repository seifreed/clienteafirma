...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  ...
  - **OID4VP completo:** perfiles finales de interoperabilidad. DCQL nativo ya sustituye al legacy `presentation_definition_uri` cuando se declara y exige `credentials`; JAR by value ya genera Request Objects firmados con `iss` y vigencia corta; `client_id`/`nonce` no pueden ir vacíos, `verifier`/`response_uri` exigen HTTPS en `EudiwProtocolHandler` y `response_mode` queda limitado a `direct_post`/`direct_post.jwt`; JARM `direct_post.jwt` ya se solicita y valida, incluidos `iss`, `exp`/`nbf`.
