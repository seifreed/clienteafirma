...
- **M4 — fase 2 (después de fase 1).** Lo que la sesión 2026-05-07 dejó pendiente:
  ...
  - **OID4VP completo:** perfiles finales de interoperabilidad. DCQL nativo ya sustituye al legacy `presentation_definition_uri` cuando se declara; JAR by value ya genera Request Objects firmados; JARM `direct_post.jwt` ya se solicita y valida, incluida vigencia `exp`/`nbf`.
