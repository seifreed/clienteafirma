...
- **M3.4-windows — repack del bundle NSS para Windows.** Ejecutar `scripts/repack-nss-windows.ps1` en un host Windows con Firefox instalado. Esto quitará la última supresión activa de `sqlite3.dll` (CVE-2021-36690). No se puede cerrar desde macOS/Linux porque Mozilla no publica binarios standalone de NSS para Windows.
- **M3.5 — fork iText → OpenPDF.** Bloqueado: `afirma-lib-itext:1.7` es un fork `com.aowagie.*` con parches PAdES propios (`PdfPKCS7.getPkcs1()`, `InvalidPageNumberException`, sobrecargas de `createSignature`/`preClose`/`PdfSignature`) que OpenPDF no trae. Falta portar esos parches a OpenPDF o mantener un fork propio.
- **M3.6 — migración JUnit 4 → Jupiter.** No bloquea release: JUnit Platform + Vintage ya ejecutan los tests existentes. Falta convertir tests JUnit 4 solo cuando se toquen por otros cambios.
- **M4 — JAdES LT/LTA.** JAdES-T básico ya existe. Falta política TSA CTT por defecto y conexión con `afirma-trust-tsl` para material LT/LTA.
- **M4 — LOTL Comisión.** `LotlLoader` ya descarga, verifica y cachea LOTL con clave pinada suministrada por integrador. Falta fijar el pin oficial del certificado de firma de la Comisión y su política de distribución/rotación CTT.
- **M4 — OID4VP/SD-JWT interoperabilidad.** DCQL, JAR, JARM y verificación SD-JWT VC básica ya están implementados. Faltan perfiles finales de interoperabilidad contra wallets reales.
- **M4 — conformance.** Falta ejecutar y documentar resultados de EU Reference Wallet + eIDAS Test Bench.
- **M4 — mobile.** `walletUri` y `walletEndpoint` ya existen. Faltan contratos cerrados con `afirma-android` y `afirma-ios` (repos CTT separados).
