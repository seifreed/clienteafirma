...
final class ExtraParamsHelper {
	...
	private static Properties loadFacturaEExtraParams() {
        ...
        final String policyId = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_POLICY_IDENTIFIER);

        if (policyId != null && !policyId.trim().isEmpty()) {
        	p.put(XAdESExtraParams.POLICY_IDENTIFIER, policyId);

        	final String policyIdHash = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_POLICY_IDENTIFIER_HASH);
        	if (policyIdHash != null && !policyIdHash.trim().isEmpty()) {
        		p.put(XAdESExtraParams.POLICY_IDENTIFIER_HASH, policyIdHash);
        	}
        	final String policyHashAlgorithm = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_POLICY_IDENTIFIER_HASH_ALGORITHM);
		...
		final String signatureCity = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_SIGNATURE_PRODUCTION_CITY);
        if (signatureCity != null && !signatureCity.trim().isEmpty()) {
        	p.put(XAdESExtraParams.SIGNATURE_PRODUCTION_CITY, signatureCity);
        }
        final String signatureProvince = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_SIGNATURE_PRODUCTION_PROVINCE);
        ...
        final String signaturePC = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_SIGNATURE_PRODUCTION_POSTAL_CODE);
        if (signaturePC != null && !signaturePC.trim().isEmpty()) {
        	p.put(XAdESExtraParams.SIGNATURE_PRODUCTION_POSTAL_CODE, signaturePC);
        }
        final String signatureCountry = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_SIGNATURE_PRODUCTION_COUNTRY);
        ...
        final String signerRole = PreferencesManager.get(SignatureFormatPreferenceKeys.FACTURAE_SIGNER_ROLE);
	...
	private static Properties loadXAdESExtraParams() {
        ...
        // Preferencias de politica de firma
        final String policyId = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_POLICY_IDENTIFIER);

        if (policyId != null && !policyId.trim().isEmpty()) {
        	p.put(XAdESExtraParams.POLICY_IDENTIFIER, policyId);

        	final String policyIdHash = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_POLICY_HASH);
        	if (policyIdHash != null && !policyIdHash.trim().isEmpty()) {
        		p.put(XAdESExtraParams.POLICY_IDENTIFIER_HASH, policyIdHash);
        	}
        	final String policyHashAlgorithm = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_POLICY_HASH_ALGORITHM);
        	if (policyHashAlgorithm != null && !policyHashAlgorithm.trim().isEmpty()) {
        		p.put(XAdESExtraParams.POLICY_IDENTIFIER_HASH_ALGORITHM, policyHashAlgorithm);
        	}
        	final String policyQualifier = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_POLICY_QUALIFIER);
        	if (policyQualifier != null && !policyQualifier.trim().isEmpty()) {
        		p.put(XAdESExtraParams.POLICY_QUALIFIER, policyQualifier);
        	}
        }

        final String claimedRole = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_SIGNER_CLAIMED_ROLE);
        if (claimedRole != null && !claimedRole.trim().isEmpty()) {
            p.put(XAdESExtraParams.SIGNER_CLAIMED_ROLES, claimedRole);
        }

		final String signatureCity = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_SIGNATURE_PRODUCTION_CITY);
        if (signatureCity != null && !signatureCity.trim().isEmpty()) {
        	p.put(XAdESExtraParams.SIGNATURE_PRODUCTION_CITY, signatureCity);
        }
        final String signatureProvince = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_SIGNATURE_PRODUCTION_PROVINCE);
        if (signatureProvince != null && !signatureProvince.trim().isEmpty()) {
        	p.put(XAdESExtraParams.SIGNATURE_PRODUCTION_PROVINCE, signatureProvince);
        }
        final String signaturePC = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_SIGNATURE_PRODUCTION_POSTAL_CODE);
        if (signaturePC != null && !signaturePC.trim().isEmpty()) {
        	p.put(XAdESExtraParams.SIGNATURE_PRODUCTION_POSTAL_CODE, signaturePC);
        }
        final String signatureCountry = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_SIGNATURE_PRODUCTION_COUNTRY);
        if (signatureCountry != null && !signatureCountry.trim().isEmpty()) {
        	p.put(XAdESExtraParams.SIGNATURE_PRODUCTION_COUNTRY, signatureCountry);
        }

        final String signFormat = PreferencesManager.get(SignatureFormatPreferenceKeys.XADES_SIGN_FORMAT);
	...
	private static Properties loadPAdESExtraParams() {
        ...
        final String policyId = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_POLICY_IDENTIFIER);
        if (policyId != null && !policyId.trim().isEmpty()) {
        	p.put(PdfExtraParams.POLICY_IDENTIFIER, policyId);

        	final String policyIdHash = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_POLICY_HASH);
        	if (policyIdHash != null && !policyIdHash.trim().isEmpty()) {
        		p.put(PdfExtraParams.POLICY_IDENTIFIER_HASH, policyIdHash);
        	}
        	final String policyHashAlgorithm = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_POLICY_HASH_ALGORITHM);
        	if (policyHashAlgorithm != null && !policyHashAlgorithm.trim().isEmpty()) {
        		p.put(PdfExtraParams.POLICY_IDENTIFIER_HASH_ALGORITHM, policyHashAlgorithm);
        	}
        	final String policyQualifier = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_POLICY_QUALIFIER);
        	if (policyQualifier != null && !policyQualifier.trim().isEmpty()) {
        ...
        // Metadatos PAdES
        final String signReason = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_SIGN_REASON);
        if (signReason != null && !signReason.trim().isEmpty()) {
        	p.put(PdfExtraParams.SIGN_REASON, signReason);
        }
        final String productionCity = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_SIGN_PRODUCTION_CITY);
        if (productionCity != null && !productionCity.trim().isEmpty()) {
        	p.put(PdfExtraParams.SIGNATURE_PRODUCTION_CITY, productionCity);
        }

        final String contact = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_SIGNER_CONTACT);
        if (contact != null && !contact.trim().isEmpty()) {
        	p.put(PdfExtraParams.SIGNER_CONTACT, contact);
        }

        // PAdES BES/Basic
        final String subfilter = PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_FORMAT);
        ...
        final boolean obfuscate = Boolean.parseBoolean(PreferencesManager.get(SignatureFormatPreferenceKeys.PADES_OBFUSCATE_CERT_INFO));
	...
	private static Properties loadCAdESExtraParams() {
        ...
        // Preferencias de politica de firma
        final String policyId = PreferencesManager.get(SignatureFormatPreferenceKeys.CADES_POLICY_IDENTIFIER);
        if (policyId != null && !policyId.trim().isEmpty()) {
        	p.put(CAdESExtraParams.POLICY_IDENTIFIER, policyId);

        	final String policyIdHash = PreferencesManager.get(SignatureFormatPreferenceKeys.CADES_POLICY_HASH);
        	if (policyIdHash != null && !policyIdHash.trim().isEmpty()) {
        		p.put(CAdESExtraParams.POLICY_IDENTIFIER_HASH, policyIdHash);
        	}
        	final String policyHashAlgorithm = PreferencesManager.get(SignatureFormatPreferenceKeys.CADES_POLICY_HASH_ALGORITHM);
        	if (policyHashAlgorithm != null && !policyHashAlgorithm.trim().isEmpty()) {
        		p.put(CAdESExtraParams.POLICY_IDENTIFIER_HASH_ALGORITHM, policyHashAlgorithm);
        	}
        	final String policyQualifier = PreferencesManager.get(SignatureFormatPreferenceKeys.CADES_POLICY_QUALIFIER);
        	if (policyQualifier != null && !policyQualifier.trim().isEmpty()) {
        ...
        p.put(
    		...
    		PreferencesManager.getBoolean(SignatureFormatPreferenceKeys.CADES_IMPLICIT) ?
    				"implicit" : //$NON-NLS-1$
					"explicit" //$NON-NLS-1$
		);
        return p;
