 ...
 *
 * <p>Requieren un DNIe real insertado en el lector y, en los tests PKCS#11/CAPI,
 * sistema Windows con la librer&iacute;a {@code DNIe_P11_x64.dll} instalada.
 * Sin la propiedad {@code afirma.it.dnie=true} los tests se omiten — no se
 * mockean — siguiendo la pol&iacute;tica "No mocks (mandatory)" del CLAUDE.md.
 *
...
class TestDnie {
	...
	@Test
	@EnabledIfSystemProperty(named = "afirma.it.dnie", matches = "true")
    void testDnieJava() throws Exception {
        ...
        for (final String alias : aliases) {
        	...
        	assertNotNull(cert, "No se pudo recuperar el certificado"); //$NON-NLS-1$
        }
	...
	@EnabledIfSystemProperty(named = "afirma.it.dnie", matches = "true")
    void testDnieCapi() throws Exception {
        Logger.getLogger("es.gob.afirma").setLevel(Level.WARNING); //$NON-NLS-1$
        ...
        for (final String alias : aliases) {
        	final X509Certificate cert = ksm.getCertificate(alias);
        	assertNotNull(cert, "No se pudo recuperar el certificado"); //$NON-NLS-1$
        	if (cert.getSubjectX500Principal().toString().contains("(FIRMA)") //$NON-NLS-1$
        			&& cert.getIssuerX500Principal().toString().contains("DNIE")) { //$NON-NLS-1$
        		signAlias = alias;
        	}
        }
        assertNotNull(signAlias);
	...
	@EnabledOnOs(OS.WINDOWS)
	@EnabledIfSystemProperty(named = "afirma.it.dnie", matches = "true")
    void testDnie64BitsPkcs11() throws Exception {
        Logger.getLogger("es.gob.afirma").setLevel(Level.WARNING); //$NON-NLS-1$
    ...
    /** Prueba de carga y uso de DNIe directamente a trav&eacute;s de su PKCS#11,
     * sin usar clases del cliente.
     * @throws Exception En cualquier error. */
	@Test
	@EnabledOnOs(OS.WINDOWS)
	@EnabledIfSystemProperty(named = "afirma.it.dnie", matches = "true")
    void testRawDniePkcs11() throws Exception {
    	...
    	final PrivateKeyEntry keyEntry = (PrivateKeyEntry) ks.getEntry(
    			SIGN_ALIAS,
    			new KeyStore.PasswordProtection(certPasswordCallback.getPassword()));
